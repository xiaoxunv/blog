...
import com.xiaowei.spring.boot.blog.repository.es.EsBlogRepository;
...
public class EsBlogServiceImpl implements EsBlogService {
	...
	public List<TagVO> listTop30Tags() {
		...
		SearchQuery searchQuery = new NativeSearchQueryBuilder()
                ...
                //把tags作为一个聚合条件来查询前30这么一个标签，完全是通过ES来实现的，ES自身就提供了向热刺统计啊，聚合啊等等一些功能
	...
	@Override
	public List<TagVO> listTop20Tags() {
		...
		SearchQuery searchQuery = new NativeSearchQueryBuilder()
				...
				.withIndices("blog").withTypes("blog")
                //把tags作为一个聚合条件来查询前30这么一个标签，完全是通过ES来实现的，ES自身就提供了向热刺统计啊，聚合啊等等一些功能
				.addAggregation(terms("tagss").field("tags").order(Terms.Order.count(false)).size(30))
				.build();
		// when
		Aggregations aggregations = elasticsearchTemplate.query(searchQuery, new ResultsExtractor<Aggregations>() {
			@Override
		...
		StringTerms modelTerms =  (StringTerms) aggregations.asMap().get("tagss");
